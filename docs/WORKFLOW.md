...
1. Check the destination **before** retrying anything: a push or a pull request creation that reported a failure may already have taken effect. `gh pr list --repo <repository> --head <branch> --base <baseBranch> --state all`, or the repository's own page, answers that.
2. If an open pull request for that head and base is already there, the attempt is published and nothing else is needed.
3. If it is missing, push the retained workspace's branch by hand and open the pull request with `gh pr create --repo <repository> --head <branch> --base <baseBranch> --title "<task id: title>" --body-file <runDir>/logs/delivery-pull-request-body.md`. The body file is the one the failed step wrote beside the run's other logs, so the pull request still carries the Jira reference, the task, the check summary, and the run ID.
