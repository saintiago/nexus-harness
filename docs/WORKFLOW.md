...
The harness's own Git steps are bounded too, and are not configuration: inside a run each one gets what is left of `taskTimeoutMinutes` and the run's own stop request, and a reading outside a run — the source preflight, a continuation's branch check, the final comparison — runs under a fixed finite bound (docs/spec.md §3). A Git stopped at its bound is reported as that stop, with whether it was confirmed.
