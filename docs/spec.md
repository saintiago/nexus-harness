...
**Revision: 2026-09-20 — one harness configuration, one file per connected project.** The configuration contract in [WORKFLOW.md](WORKFLOW.md) §1 is implemented: one Nexus-wide harness configuration an operator keeps (`--config`) owns the output directory, the limits, the coding launches, and the reviewer integration; one project configuration, committed at each connected repository's root as `nexus.project.json`, owns that repository's setup/checks, its Jira connection, and its GitHub destination. The loader composes the two, refuses a field in the file that does not own it, and refuses a project completion without the harness completion policy — before anything is claimed or run. Shared reviewer policy composes a review only for projects declaring both Jira and delivery; local-only and Jira-only projects can use the same harness file. The commands validate and print the composition (`check-config`, and the `--project` on the read-only commands), and the retired single-file configuration is no longer read.
...
- One reviewer turn is bounded by the same task timeout a run gets and runs through the same adapter inside the pinned checkout at `<workDir>/reviews/<reviewId>/repo/`, with the ordinary Git repository check enabled. Its prompt directs it to read the applicable `AGENTS.md` files as review evidence; repository content cannot authorize fixes or publication. Its verdict and logs stay in the parent evidence directory. It is review-only: it must not implement fixes, change the view, commit, push, merge, or edit the ticket or the pull request. It must write one verdict file naming `approve`, `request_changes`, or `inconclusive` with a summary and findings. An explicit inconclusive result explains missing material evidence and publishes no review or check. Findings are blocking; approval requires an empty findings list and sufficient evidence.
