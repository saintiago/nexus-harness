...
- Before anything is published, the repository view must still be the clean snapshot pinned at the reviewed head — an edit, a new file, a commit, or a moved head in the view publishes nothing — the pull request is re-read and must still be open at that same head, and the ticket must still be in the configured review status. A head that moved, a ticket that left review, and a view the turn changed publish nothing: a stale verdict can never approve a newer commit.
