...
**Revision: 2026-09-20 — a repair turn stays on the recorded branch.** A coding turn can commit on a branch of its own and leave the checkout there. Before every coding turn, and before the round that judges it, the checkout is returned to the branch its workspace ledger records when that can be done without losing anything: a clean checkout whose commit descends from that branch is fast-forwarded to it and checked out, so a continuation and a repair turn work on the branch the workspace owns, and the checks and a delivery step are about one revision. A dirty, detached, divergent, or branchless checkout stops before that turn or check, naming the branches and the manual action; nothing is reset, force-updated, or discarded, and §7's exact-revision delivery refusal is unchanged. The workspace contract is in [implement-workspace-continuation.md](implement-workspace-continuation.md).
