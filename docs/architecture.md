...
**Revision: 2026-09-21 — a completed red baseline is diagnosed before any developer turn.** A
fresh workspace whose configured setup succeeded and whose check round completed nonzero is handed
to one bounded local diagnosis instead of being published as a plain failure. Three small modules
own it: `sources/baseline.ts` is the phase — the evidence identity, the marker, the retained
evidence record a restart resumes from, the one comment and the one move — `reviews/baseline.ts` is
the one reviewer turn over a read-only clone of the snapshot (reusing `reviews/view.ts`), the
working copy and clone checks that hold the turn to the snapshot it was given, and the finding file
it must write, and
`sources/jira/baseline.ts` is the thin Jira side (the thread, one comment, one move out of the
running status, and whether the item is still there) over the completion path's existing comment
...
and finishes whatever a previous invocation left pending before it discovers or claims anything,
  ...
  at, so a field one bounded comment line cannot hold still reaches the developer, and that width is
  never charged against the bounds the rest of the guidance is kept to, so a long finding cannot
  crowd out the newest thing the ticket says — a finding no source can supply — otherwise
...
yet, so nothing is fabricated for one. Its evidence lives under the connected project's own
...
`sources/baseline.ts` is the one pre-delivery diagnosis: given one completed red baseline on a fresh workspace, it records the evidence locally (`<workDir>/baseline/<project>/<evidence>/evidence.json`, `<project>` being the connected project's own namespace), reads the item's own thread for the finding it already carries, runs one reviewer turn through the `BaselineReview` boundary only when neither the thread nor that evidence holds one, writes one comment, and makes the one status move the finding asks for. The marker on the thread names the evidence and nothing else: it is held against the outcome the reviewer turn's own record holds, so only an accepted actionable finding returns the item for repair and closes the evidence as one — an edited marker on a rejected turn never can — and a stop that reaches the phase before it published anything is bounded feedback and a move out of the running status for the claimed ticket instead of a ticket left with nothing looking for it. Its `resume` step finishes what a stopped invocation left behind — the missing move, or the publication of the validated outcome the reviewer turn recorded — before anything is discovered or claimed, and it never spends a second reviewer turn for the same evidence or touches an item a person has moved; when it finds the item already out of the running status it reconciles the retained record with the finding the item's own thread carries — settled only as the recorded outcome really says, so a rejection beside it never finishes the evidence as the repair a marker claims — and a publication that succeeded before the local record was finished still reaches that workspace's next claim. It reads only its own project's evidence directory, and a record naming another project is refused by name rather than acted on. An evidence directory this harness kept whose own record is gone is not "nothing pending" either: nothing about it can be resumed, read back, or closed, and it may be the record that returned a workspace for repair, so intake stops for a person — naming the directory — instead of being passed over or claimed on as an ordinary continuation. `sources/jira/baseline.ts` implements the record it writes through — the thread, one comment, one move out of the running status, and whether the item is still there — over `sources/jira/completion.ts`'s comment reader and its general status move. `sources/coordinator.ts` decides which ending qualifies, hands the phase the item, the retained workspace and the round, runs the resume step before discovery in a batch, a watch scan and a queue step, then publishes nothing else for that run: an actionable finding goes back to the ready status for the ordinary ladder, and a diagnosis with nothing actionable leaves the item In Review and stops intake for a person — keeping the intake lock when the diagnosis reports a reviewer runtime it could not confirm stopped. It also requires the finding before it starts that continuation — from the thread when the read succeeds and carries it, otherwise read back from the retained evidence, otherwise intake stops rather than starting a developer without it. `sources/guidance.ts` carries a reviewed finding into a later attempt field by field, from either source, and widens nothing else about the context a turn is given. That finding's own width was bounded where its turn was accepted and is never charged against the bounds the rest of the guidance is kept to, so a long finding cannot crowd out the newest thing the ticket says. `reviews/baseline.ts` reads the output the failing checks wrote strictly for this diagnosis: a log file that cannot be read is incomplete evidence named by its path, kept apart from a log the command left empty, and no reviewer turn is started from it. It also records what the one turn produced — the validated finding, or the problem that rejected it, with the turn's own stop — in `outcome.json` before anything is published, and every reader of what that evidence produced goes through that record rather than the turn's finding file, which cannot tell a completed turn from one that failed: the marker a resume holds, the reconciliation of a record left unfinished, and the finding a continuation is required to be told all agree with it, so a rejected turn's own file is never handed to a developer.
...
The pre-delivery baseline diagnosis is the one turn that is not a coding turn, and it names the narrower policy instead: `exec --sandbox workspace-write`, started in its own working directory inside the diagnosis's evidence directory, with the policy's additional writable roots stated as none and the host's temporary roots excluded from its writable set (`runtime.ts`). That is the same adapter — one launch interface, one place where a sandbox policy is chosen — with the writable root moved off the trees the turn must not change: the snapshot clone and the ticket's retained working copy stay read-only to it however `workDir` is placed and whatever the operator's own configuration grants, and the reviewer's one finding file is written in the directory it was started in (`reviews/baseline.ts`). Those configuration keys are not the only place a configured launch prefix can widen a launch, so the adapter refuses a diagnostic prefix that carries a switch of its own (`--add-dir`, `--cd`/`-C`, `--worktree`, `-s`/`--sandbox`, `--dangerously-bypass-approvals-and-sandbox`) before it starts anything: a grant the launch cannot take back is never handed to a runtime, and the refusal is the turn's failure, which the diagnosis records on the ticket In Review (`runtime.ts`, §11 of [spec.md](spec.md) and [WORKFLOW.md](WORKFLOW.md)).
...
Use the configured project/type/label/ready status to build a safely quoted JQL expression, closed by the one clause the configured `ordering` selects: `ORDER BY priority DESC, created ASC, key ASC` by default, or `ORDER BY Rank ASC, created ASC, key ASC` in rank mode. Jira performs that ordering — its native Rank is the primary order, and its values are never fetched or reinterpreted here — and the connector keeps the answer's order across pages rather than re-sorting locally, so a Priority or Rank change affects only a later scan. Request only needed search fields. Follow `nextPageToken` until `isLast`; reject a missing/repeated continuation token on a nonfinal page. Never use a returned `total`, old `startAt` pagination, or a fixed first page as the complete queue. This API may return stale search data; use current issue reads as additional checks. [A2]

A site that refuses Rank JQL reports its own bounded error and starts no task: no fallback to Priority, no Agile board API, no Rank write. The In Review queue the completion path scans keeps its own fixed deterministic order; `ordering` is the ready queue's intake choice.
