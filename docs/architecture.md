...
own it: `sources/baseline.ts` is the phase — the evidence identity, the marker, the one comment and
the one move — `reviews/baseline.ts` is the one reviewer turn over a read-only clone of the
snapshot (reusing `reviews/view.ts`) and the finding file it must write, and
`sources/jira/baseline.ts` is the thin Jira side (the thread, one comment, one move out of the
running status) over the completion path's existing comment and transition helpers.
...
`sources/baseline.ts` is the one pre-delivery diagnosis: given one completed red baseline on a fresh workspace, it reads the item's own thread for the evidence it already carries, runs one reviewer turn through the `BaselineReview` boundary when there is none, writes one comment, and makes the one status move the finding asks for. `sources/jira/baseline.ts` implements the record it writes through — the thread, one comment, one move out of the running status — over `sources/jira/completion.ts`'s comment reader and its general status move. `sources/coordinator.ts` decides which ending qualifies and hands the phase the item, the retained workspace and the round, then publishes nothing else for that run: an actionable finding goes back to the ready status for the ordinary ladder, and a diagnosis with nothing actionable leaves the item In Review and stops intake for a person.
