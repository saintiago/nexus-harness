...
yet, so nothing is fabricated for one. [spec.md](spec.md) §11 defines the behavior,
...
`sources/baseline.ts` is the one pre-delivery diagnosis: given one completed red baseline on a fresh workspace, it records the evidence locally (`<workDir>/baseline/<evidence>/evidence.json`), reads the item's own thread for the finding it already carries, runs one reviewer turn through the `BaselineReview` boundary only when neither the thread nor that evidence holds one, writes one comment, and makes the one status move the finding asks for. Its `resume` step finishes what a stopped invocation left behind — the missing move, or the publication of a finding the reviewer turn already wrote — before anything is discovered or claimed, and it never spends a second reviewer turn for the same evidence or touches an item a person has moved. `sources/jira/baseline.ts` implements the record it writes through — the thread, one comment, one move out of the running status, and whether the item is still there — over `sources/jira/completion.ts`'s comment reader and its general status move. `sources/coordinator.ts` decides which ending qualifies, hands the phase the item, the retained workspace and the round, runs the resume step before discovery in a batch, a watch scan and a queue step, then publishes nothing else for that run: an actionable finding goes back to the ready status for the ordinary ladder, and a diagnosis with nothing actionable leaves the item In Review and stops intake for a person. `sources/guidance.ts` carries a reviewed finding into a later attempt field by field, and widens nothing else about the context a turn is given.
