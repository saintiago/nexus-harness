...
- **Green CI.** Two workflows run `npm ci` and `npm run validate`: `ci.yml` on every push and pull
  request, and `auto-pr.yml` on a push to a `task/**` branch, as the gate in front of its own
