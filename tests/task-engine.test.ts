...
describe('TaskEngine over the finite workflow', () => {
  ...
  it(
    'faults instead of hanging when an active snapshot lost its invoked child',
    { timeout: 2000 },
    async () => {
      const stateFile = await temporaryStateFile();
      const interrupted = suppliedActions({
        SelectTask: async () => 'selected',
        CompleteTask: async () => {
          throw new Error('completion service unavailable');
        },
      ...
      await createTaskEngine({
        workflow: finiteDelivery,
        stateFile,
        bindActions: () => interrupted.actions,
      }).run();

      const snapshot = await persistedState(stateFile);
      expect(snapshot).toMatchObject({
        status: 'active',
        value: 'complete',
        children: { '0.finite-delivery.complete': { src: 'CompleteTask' } },
      });
      // A valid active snapshot with only the child driving the active invocation removed.
      await writeFile(stateFile, JSON.stringify({ ...snapshot, children: {} }), 'utf8');

      const resumed = suppliedActions();
      const result = await createTaskEngine({
        workflow: finiteDelivery,
        stateFile,
        bindActions: () => resumed.actions,
      }).run();

      expect(result.ok).toBe(false);
      expect(result.ok ? '' : result.fault.message).toMatch(/cannot be restored/);
      expect(result.ok ? '' : result.fault.message).toContain('CompleteTask');
      expect(resumed.calls).toEqual([]);
    },
  );

  it(
    'faults on malformed active snapshots instead of starting over',
    { timeout: 2000 },
    async () => {
      const cases: ReadonlyArray<readonly [string, unknown, RegExp]> = [
        [
          'a missing state value',
          { status: 'active', context: {}, children: {} },
          /cannot be restored/,
        ],
        [
          'a non-state value',
          { status: 'active', value: 7, context: {}, children: {} },
          /cannot be restored/,
        ],
        [
          'children that are not a snapshot map',
          { status: 'active', value: 'complete', context: {}, children: [] },
          /CompleteTask/,
        ],
      ];
      ...
      for (const [label, snapshot, expected] of cases) {
        const stateFile = await temporaryStateFile();
        await writeFile(stateFile, JSON.stringify(snapshot), 'utf8');
        const { actions, calls } = suppliedActions();
        ...
        expect(result.ok ? '' : result.fault.message, label).toMatch(expected);
