 ...
 *
 * The reviewer turn that inspects a completed red baseline and writes one structured finding, the evidence it reads, the finding file it validates, the comment the harness publishes, the Jira record of it, and the coordinator that decides whether a completed red baseline enters the diagnosis at all. The queue-facing cases that continue from a diagnosis live in tests/baseline-queue.integration.test.ts.
 */
...
import type { CheckRoundResult } from '../src/shared/types.js';
