...
export function recoveryPrompt(brief: RecoveryBrief): string {
  ...
  sections.push(
    [
      ...
      `- Baselines the queue diagnosed: \`baseline/<project>/<evidence>/evidence.json\` under that`,
      '  directory, with the reviewer outcome beside it. An item left in the running status by an',
      '  interrupted diagnosis — a red baseline diagnosed but never finished — is an interrupted',
      '  episode that only you reconcile: the ordinary queue gains no branch for it and never guesses',
      '  what such a record holds. Read the evidence, the workspace and the ticket, bring the item',
      '  back to a state the queue can carry, and say what resumes.',
  ...
  sections.push(
    [
      ...
      '   The parent verifies that blocker against the ticket itself: the interrupted work resumes',
      '   only once that ticket has really reached the configured done status, and a blocker that',
      '   ends up somewhere the queue does not carry stops for a person instead of resuming.',
