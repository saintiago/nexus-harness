...
export function createBriefWriter(settings: BriefWriterSettings): BoundAction {
  return async () => {
    ...
    const context = [
      ...
      'and keep its proposed concept and direction rather than replacing it with a different or',
      'more generic idea. State the refined idea in clear parts: `idea` states the desirable',
      'change, why it matters and the principle behind it, without committing to implementation;',
      '`projectFit` states why it belongs in this project; `feasibility` states a plausible path',
      'given the known constraints and evidence, not a design or implementation plan; and',
      '`openQuestions` lists only the material questions the next workflow must answer, omitting',
      'them when there are none. Keep detailed research in the research artifact and cite it',
      'selectively; the refined idea stays concise and on point. Do not prescribe implementation',
      ...
      'of design tradeoffs. Keep council history out of the idea\u2019s parts; changeSummary is the',
      'cumulative account of what refinement changed. The council reviews this revision for',
      'project fit, coherent value, fidelity to the author\u2019s intent, evidence quality, fairly',
      'represented alternatives and the smallest useful scope.',
