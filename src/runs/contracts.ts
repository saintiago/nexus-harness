...
export interface RunTaskResult {
   ...
   * so a save that failed is never treated as one that succeeded: the run's
   * report, logs, and working copy are kept as they were written, this field
   * names the ledger path and what failed, and a caller that would continue the
   * workspace automatically — the source intake's escalation ladder above all —
   * must stop instead of starting another attempt against a ledger that does not
   * hold this one (docs/spec.md §6).
   */
